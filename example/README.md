...
- Find/Replace `nosql_demo` to name of the flutter name of the app (ex: `my_spiffy_app`)
- Find/Replace `NoSQL Demo` to a label used to name the app on mobile Platforms (ex:`My Spiffy Mobile App`)
- Find/Replace `NoSQL Demo for Mac` to the name of your macOS app (ex: `My Spiffy macOS App`)
...
- Find/Replace `nosql_demo` with bundle id (ex: `my_spiffy_app`) **[using the folder name is ideal]**
- Find/Replace `nosqlDemo` for Apple bundles as iOS/MacOS do like '_'. **[ideal is folder name to lower-camel case (ex: `mySpiffyApp`)]**
- The image `/assets/images/nosql1024.png` is a placeholder image to generate application icons. In the `pubspec.yaml` file is section marked *flutter_icons*:
  ...
  NOTE: The sample app already has the [nosql1024](assets/images/nosql1024.png). The best images are 1024x1024. Replace the image and make sure the `image_path` in `pubspec.yaml` is updated and then run the
